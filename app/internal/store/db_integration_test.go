...
func TestPutEmployee(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			ExpectedErr: errors.New(`failed to store employee: ERROR: null value in column "position" of relation "employees" violates not-null constraint (SQLSTATE integration_tests)`),
		},
	}
